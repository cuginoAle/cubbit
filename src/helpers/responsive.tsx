...
export const SmallScreenOnly: React.FC = ({ children }: Props) => {
  ...
  const isSmall = useMediaQuery({ query: `(${themeContext.breakPoints.small})` })
  return (
    <>
      {isSmall && children}
    </>
...
export const LargeScreenOnly: React.FC = ({ children }: Props) => {
  const themeContext = useContext(ThemeContext);
  const isLarge = useMediaQuery({ query: `(${themeContext.breakPoints.large})` })
  return (
    <>
      {isLarge && children}
    </>
...
export const useCheckMQ = (mq: string): boolean => useMediaQuery({ query: `(${mq})` })